...
func (r *Registration) Walk(id any) (reg *Registration) {
	...
	case string:
		if _, a, err := cleanASN1(tv); err != nil {
			if dot := trimL(tv, `.`); oid.IsDotNotation(dot) {
...
instances of [go-radua/Cache] when, and when not, to cache an instance.
...
COLLECTIVE TTL.
*/
func (r *Registration) CTTL() string {
	return r.RC_TTL
}
